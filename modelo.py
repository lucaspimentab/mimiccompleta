...
# 5. Métricas globais
print(f"🧠 AUC ROC (cross-val): {roc_auc_score(y, y_prob_cv):.4f}")
print(f"📉 Matriz de Confusão:\n{confusion_matrix(y, y_pred_cv)}")
report = classification_report(y, y_pred_cv, target_names=["Sobrevivente", "Óbito"], output_dict=True)
report_df = pd.DataFrame(report).drop(columns=["macro avg", "weighted avg", "accuracy"]).round(2)
print("\n📊 Classification Report:")
print(report_df.to_string())
print(f"\n🎯 Acurácia: {report['accuracy']:.2f}")

# 6. Matriz de confusão (imagem)
...
# 7. SHAP - Importância das variáveis ordenadas
...
# 8. Exportar falsos positivos e negativos
